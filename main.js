...
async function findFlips(HIDE_FURNITURE, HIDE_PET_SKINS, HIDE_DUNGEON_ITEMS, HIDE_DRAGON_ITEMS, PROFITMIN, PERMIN, STOCKMIN, SORTTYPE, pages){
    ...
    for (let i = 0; i < pages; i++){
        ...
        j.auctions.forEach(auction => {
            if(sex < 30 && auction["bin"]!=false && auction["item_name"].includes("Divan") && (auction["item_name"].includes("Chestplate") || auction["item_name"].includes("Boots") || auction["item_name"].includes("Leggings") || auction["item_name"].includes("Helmet")) && (!(auction["item_lore"].includes("§8⸕§8")) || !(auction["item_lore"].includes("§8⸕§8")) || !(auction["item_lore"].includes("§8✧§8")) || !(auction["item_lore"].includes("§8⸕§8")) || !(auction["item_lore"].includes("§8☘§8"))) && auction["starting_bid"]<42000000){
                console.log(auction)
